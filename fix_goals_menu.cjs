...
code = code.replace(/<motion\.div\s*key="goal-menu"\s*initial=\{\{ opacity: 0, scale: 0\.92, y: -6 \}\}\s*animate=\{\{ opacity: 1, scale: 1, y: 0 \}\}\s*exit=\{\{ opacity: 0, scale: 0\.92, y: -6 \}\}\s*transition=\{\{ duration: 0\.15, ease: "easeOut" \}\}\s*onClick=\{\(e\) => e\.stopPropagation\(\)\}\s*className="absolute right-0 top-9 bg-\[#282828\] border border-\[#4f4f4f\] rounded-\[16px\] p-1\.5 z-50 flex flex-col min-w-\[160px\]"\s*>/, 
  `<motion.div
                            key="goal-menu"
