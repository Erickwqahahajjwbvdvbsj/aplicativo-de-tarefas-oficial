...
old_modal = r"""          <motion\.div key="stageTaskModalOverlay"
            initial=\{\{ opacity: 0 \}\}
            animate=\{\{ opacity: 1 \}\}
            exit=\{\{ opacity: 0 \}\}
...
text = re.sub(old_modal, new_modal, text, flags=re.MULTILINE)
...
text = re.sub(old_end, new_end, text, flags=re.MULTILINE)
