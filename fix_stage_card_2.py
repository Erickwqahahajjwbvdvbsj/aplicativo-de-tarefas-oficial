...
new_card = """                goalStages.map((stage, idx) => (
                  <div key={stage.id} 
                       className="bg-[#282828] rounded-[14px] px-4 py-2 min-h-[76px] flex flex-col justify-center relative z-10 shrink-0 cursor-pointer"
                       onClick={() => setEditingStageId(stage.id)}
                  >
                     ...
                     {stage.title && (
                         <p className="text-white font-normal text-[15px] leading-[22px] mt-1 mb-1 whitespace-normal break-words">
                             {stage.title}
                         </p>
                     )}
                     {stage.tasks.length > 0 && (
                         <span className="text-[#a0a0a0] text-[12px] mb-1 block">
                             {stage.tasks.length} tarefa{stage.tasks.length !== 1 ? 's' : ''}
                         </span>
                     )}
                  </div>
...
if old_card in text:
    print("Found exact old card")
else:
    print("Old card not found, checking with regex")
