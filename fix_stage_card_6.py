                     ...
                     <div className="flex justify-between w-full items-center">
                        ...
                        <div className="relative flex items-center justify-center -my-1">
                            ...
                            {openStageMenuId === stage.id && (
                                <div className="absolute right-0 top-6 w-[120px] bg-[#2c2c2c] rounded-[10px] shadow-lg border border-[#4f4f4f] overflow-hidden z-[100]">
                                    <button 
...
new_card = """                goalStages.map((stage, idx) => (
                  ...
                  >
                     <div className="flex justify-between w-full items-start">
                        <span className="text-white font-bold text-[16px] leading-[22px] flex items-center min-h-[22px]">Etapa {idx + 1}</span>
                     ...
                     {stage.title && (
                         <p className="text-white font-normal text-[15px] leading-[22px] mt-1.5 whitespace-normal break-words">
                             {stage.title}
                         </p>
                     )}
                     {stage.tasks.length > 0 && (
                         <span className="text-[#a0a0a0] text-[12px] mt-1.5 block">
                             {stage.tasks.length} tarefa{stage.tasks.length !== 1 ? 's' : ''}
                         </span>
                     )}
                  </div>
